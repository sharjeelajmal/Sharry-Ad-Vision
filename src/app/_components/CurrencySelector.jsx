...
const CurrencySelector = ({
  ...
  useEffect(() => {
    ...
    // Admin hamesha fresh data fetch karega
    if (isAdmin) {
        fetchAndSetCurrencies();
    } 
    // Client page conversionRates se data lega jo pehle he fetch ho chuka hai
    else {
        const currencyList = Object.keys(conversionRates).map(code => ({
            code,
  ...
  const handleSaveCurrencies = async () => {
    try {
      const validCurrencies = currencies
        ...
        .map(({ _id, ...rest }) => rest); // Temporary _id ko remove karein
  ...
  if (isLoading && isAdmin) { // Loading state sirf admin panel par dikhayein
    return (
      ...
      {isAdmin && (
        <div className="flex flex-wrap gap-2 mb-2 justify-center">
          {isEditing ? (
            ...
            <button className="btn btn-sm btn-primary text-white" onClick={() => setIsEditing(true)}>Edit Currencies</button>
          )}
        </div>
      )}
      ...
      <AnimatedSection>
        {isEditing && isAdmin ? (
                ...
                {currencies.map((currency, index) => (
                  <tr key={currency._id || index} className="hover:bg-gray-50">
                    <td className="border px-4 py-2">
                      <input type="text" className="input input-sm input-bordered w-full max-w-[80px] bg-white" value={currency.code}
                        onChange={(e) => {
                    ...
                    <td className="border px-4 py-2">
                      <input type="text" className="input input-sm input-bordered w-full max-w-[120px] bg-white" value={currency.name}
                        onChange={(e) => {
                          const newCurrencies = [...currencies];
                    ...
                    <td className="border px-4 py-2">
                      <input type="text" className="input input-sm input-bordered w-full max-w-[60px] bg-white" value={currency.symbol}
                        onChange={(e) => {
                          const newCurrencies = [...currencies];
                    ...
                    <td className="border px-4 py-2">
                      <input type="number" className="input input-sm input-bordered w-full max-w-[100px] bg-white" value={currency.rate} step="0.0001"
                        onChange={(e) => {
                          const newCurrencies = [...currencies];
                    ...
                    <td className="border px-4 py-2">
                      <button className="btn btn-xs btn-error text-white" onClick={() => setCurrencies(currencies.filter((_, i) => i !== index))}>Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="relative inline-block text-left w-full sm:w-auto">
            <Button type="button" className="w-full sm:w-auto m-1 bg-gradient-to-r from-teal-400 to-blue-600 text-white shadow-lg flex items-center justify-center space-x-2 px-4 py-2" onClick={() => setIsOpen((prev) => !prev)}>
              ...
              <ul className="dropdown-content z-[1] menu p-2 shadow bg-white rounded-box w-full sm:w-48 absolute mt-1 right-0 max-h-60 overflow-y-auto">
                {currencies.map((currency) => (
                  <li key={currency.code}>
                    <button className={`flex items-center space-x-2 w-full text-left px-3 py-2 rounded hover:bg-gray-100 ${selectedCurrency === currency.code ? "bg-blue-50 font-semibold" : ""}`}
                      onClick={() => {
