...
const countryCurrencyMap = { PK: "PKR", US: "USD", IN: "INR", AE: "AED", GB: "GBP", DE: "EUR", FR: "EUR" };
...
const Sharry326 = () => {
  ...
  const [currentService, setCurrentService] = useState({ title: "", description: "", price: 0, quantity: "", imageUrl: "", category: "Tiktok" });
  ...
  const fetchAllInitialData = useCallback(async () => {
    ...
    try {
      ...
      setServices([]); setTabs([]); setEditTabs([]); setGalleryItems([]);
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  if (!isClient || isLoading) {
    return <div className="flex justify-center items-center h-screen"><img src="/loader326.gif" alt="loading" width={140} height={140} /></div>;
  }
  ...
  const handleSaveService = async () => {
    try {
      const response = await fetch(currentService._id ? `/api/services?id=${currentService._id}` : "/api/services", {
...
const handleGalleryDelete = async (media) => {
  ...
  return (
    <section className="mb-10">
      <div className="my-8 p-4 border rounded-lg"><h2 className="text-2xl font-bold mb-4">Popup Messages</h2><NotificationPopup isAdmin={true} /></div>
      ...
      <div className="my-8 p-4 border rounded-lg max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-4">
          ...
          <label className={`btn btn-primary text-white ${uploading ? 'loading' : ''}`}>{uploading ? 'Uploading...' : 'Upload New Media'}<input type="file" hidden accept="image/*,video/mp4,video/webm" onChange={handleGalleryUpload} disabled={uploading} /></label>
        </div>
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-4 max-h-96 overflow-y-auto">
          {galleryItems.map(media => (
            <div key={media._id} className="relative group border rounded-lg p-1">
              {media.filetype === 'video' ? (<video src={media.url} muted loop autoPlay playsInline className="w-full h-24 object-cover" />) : (<img src={media.url} alt={media.filename} className="w-full h-24 object-cover" />)}
              <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => handleGalleryDelete(media)} className="text-white bg-red-600 rounded-full p-2"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0V6z"/><path fillRule="evenodd" d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1v1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4H4.118zM2.5 3V2h11v1h-11z"/></svg></button>
              </div>
            </div>
          ))}
        </div>
      </div>
      <AnimatedSection><h1 className="font-bold text-4xl text-center py-5">Our Services</h1><StatsCards isAdmin={true} isEditing={isEditingStats} setIsEditing={setIsEditingStats}/></AnimatedSection>
      <h2 className="text-center my-7 sticky top-0 z-40"><span className="bg-gradient-to-r from-teal-400 via-cyan-500 to-blue-600 text-white shadow-lg rounded-lg py-2 px-4 font-semibold text-sm">Check Service Description? Just Click the Icon!</span></h2>
      ...
      <AnimatedSection>
        <div className="rounded-lg px-6 bg-white w-full max-w-4xl mx-auto">
          <div className="flex justify-between items-center mb-4">
            ...
            {isEditingTabs ? (<div className="flex space-x-2"><button className="btn btn-md btn-success text-white" onClick={handleSaveTabs}>Save</button><button className="btn btn-md btn-error text-white" onClick={() => { setIsEditingTabs(false); setEditTabs([...tabs]); }}>Cancel</button><button className="btn btn-md btn-primary text-white" onClick={handleAddTab}>Add Tab</button></div>) : (<button className="btn btn-md btn-primary text-white" onClick={() => setIsEditingTabs(true)}>Edit Categories</button> )}
          </div>
          {isEditingTabs ? (<div className="space-y-3 w-full">{editTabs.map((tab, index) => (<div key={index} className="flex items-center space-x-3"><input type="text" className="input input-md w-full bg-gray-50 border" value={tab} onChange={(e) => { const newTabs = [...editTabs]; newTabs[index] = e.target.value; setEditTabs(newTabs); }} /><button className="btn btn-md btn-error text-white" onClick={() => setEditTabs(editTabs.filter((_, i) => i !== index))}>×</button></div>))}</div>) : (<div className="flex flex-wrap gap-3 justify-center ">{tabs.map((tab) => (<Button key={tab} className={`btn-md px-5 font-semibold hover:bg-gray-300 shadow-equal ${activeTab === tab ? "bg-gradient-to-r from-teal-400 via-cyan-500 to-blue-600 text-white" : "bg-white text-black no-border-tab "} capitalize rounded-lg`} onClick={() => handleTabClick(tab)}>{tab}</Button>))}</div>)}
          <div className="mt-5 relative"><input type="text" placeholder="Search services..." className="input w-full bg-white border-2 border-gray-200 outline-none px-5 py-3 rounded-full shadow-sm focus:scale-105 focus:border-transparent transition-all duration-200" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /><svg className="absolute right-4 top-3.5 h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg></div>
        </div>
      </AnimatedSection>
      <div className="flex justify-end px-6 pt-6 pb-4 space-x-2"><button className="btn btn-primary text-white" onClick={handleAddService}>Add Service</button><button className="btn btn-success text-white" onClick={handleSaveOrder}>Save Order</button></div>
              ...
              <SortableServiceItem key={service._id} service={service}>
                <AnimatedSection>
                  <div className="group relative hover:scale-105 transition-all border p-3 rounded-lg max-w-xs mx-auto">
                    <div className="relative h-40 w-full mb-3">{service.imageUrl && (service.imageUrl.endsWith('.mp4') || service.imageUrl.endsWith('.webm')) ? (<video src={service.imageUrl} autoPlay loop muted playsInline className="object-contain h-full w-full" />) : (<img src={service.imageUrl || '/placeholder.png'} alt={service.title} className="object-contain h-full w-full" />)}</div>
                    <h2 className="font-bold text-xl mt-2 text-center">{service.title}</h2>
                    <p className="text-center text-sm text-gray-600">Price: {convertPrice(service.price || 0)} {selectedCurrency}{service.quantity && (<span className="inline"> {convertQuantityString(service.quantity)}</span>)}</p>
                  ...
                  <div className="flex justify-center mt-4 space-x-2"><button className="btn btn-sm text-white bg-black" onPointerDown={(e) => { e.stopPropagation(); e.preventDefault(); handleEditService(service); }}>Edit</button><button className="btn btn-sm btn-error text-white" onPointerDown={(e) => { e.stopPropagation(); e.preventDefault(); handleDeleteService(service._id); }}>Delete</button></div>
                </AnimatedSection>
              </SortableServiceItem>
            ))}
          </SortableContext>
        ...
        <dialog open className="modal modal-open">
          <div className="modal-box max-w-3xl bg-white">
            ...
            <div className="py-4 space-y-4">
              ...
              {previewUrl && (<div className="mt-2 p-2 border rounded-md w-32"><p className="text-sm text-gray-500 mb-1">Preview:</p>{previewUrl.includes('.mp4') || previewUrl.includes('.webm') ? (<video src={previewUrl} autoPlay loop muted playsInline className="max-h-40 object-contain mx-auto" />) : (<img src={previewUrl} alt="Preview" className="max-h-40 object-contain mx-auto" />)}</div>)}
              ...
              <select className="select select-bordered w-full bg-white" name="category" value={currentService.category} onChange={handleInputChange}>{tabs.map((tab) => (<option key={tab} value={tab}>{tab}</option>))}</select>
            </div>
            <div className="modal-action"><Button className="text-white bg-red-700 hover:bg-red-500" onClick={() => setIsModalOpen(false)}>Cancel</Button><Button className="btn-primary" onClick={handleSaveService}>Save</Button></div>
          </div>
        </dialog>
      )}
      {isGalleryOpen && (
