...
const Sharry326 = () => {
    const { data: session, status } = useSession(); // Session hasil karein
  ...
  const pkrRegex = /(\d{1,3}(?:,?\d{3})*|\d+)\s*PKR/gi;
    ...
    useEffect(() => {
    // Agar session check ho raha hai, to loading dikhayein
    if (status === "loading") {
      setIsLoading(true);
      return;
    }
    ...
    // Agar user login hai, to data fetch karein
    if (session) {
      setIsClient(true);
      fetch('/api/socket'); 
      fetchAllInitialData();
    ...
    if (confirm(`Delete ${media.filename}?`)) {
      try {
        ...
        setGalleryItems((prevItems) =>
          prevItems.filter((item) => item._id !== media._id)
        );
 ...
 <div className="px-4 mt-4 sm:px-6 py-4 bg-white border border-gray-200 rounded-lg shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
  ...
  />
      ...
      <AnimatedSection>
        ...
        {/* ▼▼▼ YAHAN notifyClients PASS KIYA GAYA HAI ▼▼▼ */}
        <StatsCards
          isAdmin={true}
