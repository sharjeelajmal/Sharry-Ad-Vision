...
const Sharry326 = () => {
  ...
  const [currentService, setCurrentService] = useState({
    title: "", description: "", price: 0, quantity: "", imageUrl: "", category: "",
  });
  ...
  const fetchAllInitialData = useCallback(async () => {
    ...
    try {
      ...
      const rates = fetchedCurrencies.reduce((acc, curr) => {
        acc[curr.code] = curr.rate; return acc;
      }, { PKR: 1 });
      setConversionRates(rates);
      
      if (fetchedCurrencies.length > 0) setSelectedCurrency(fetchedCurrencies[0].code || "PKR");
      
      if (serviceTabs.length > 0 && !activeTab) {
  ...
  // =================================================================
  // ===== CHANGE START: Optimized Functions (No Reload on Save) =====
  // =================================================================
  ...
  const handleSaveService = async () => {
    ...
    try {
      const response = await fetch(
        currentService._id ? `/api/services?id=${currentService._id}` : "/api/services",
        {
      ...
      if (!response.ok) {
        throw new Error(savedService.details || savedService.error || "Service save nahi ho saki.");
      }
      
      if (currentService._id) {
        // Update existing service in the state
        setServices(prevServices => prevServices.map(s => s._id === savedService._id ? savedService : s));
      } else {
  ...
  const handleDeleteService = async (id) => {
    if (confirm("Are you sure?")) {
      try {
        ...
        // Remove the service from the local state instead of refetching
  ...
  const handleSaveTabs = async () => {
    try {
      const response = await fetch("/api/tabs", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(editTabs) });
  ...
  // ===============================================================
  // ===== CHANGE END: Optimized Functions (No Reload on Save) =====
  // ===============================================================
  
  const handleAddService = () => {
    setCurrentService({
      title: "", description: "", price: 0, quantity: "", imageUrl: "", category: activeTab || (tabs[0] || ""),
    });
    setPreviewUrl("");
    setIsModalOpen(true);
  };
  ...
  const handleSaveOrder = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/services/reorder", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(orderPayload) });
  ...
  const handleGalleryDelete = async (media) => {
    if (confirm(`Delete ${media.filename}?`)) {
      try {
        const response = await fetch("/api/api/gallery", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: media._id, url: media.url }) });
  ...
  const handleSaveSettings = async () => {
    if (!whatsappNumber) { toast.error("WhatsApp number cannot be empty."); return; }
    toast.loading("Saving settings...");
    try {
      const res = await fetch("/api/settings", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ whatsappNumber }) });
  ...
  return (
    <section className="mb-10 px-4">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-4 max-h-96 overflow-y-auto">
          {galleryItems.map((media) => (
            <div key={media._id} className="relative group border rounded-lg p-1">
              {media.filetype === "video" ? ( <video src={media.url} muted loop autoPlay playsInline className="w-full h-32 object-cover" /> ) : ( <img src={media.url} alt={media.filename} className="w-full h-32 object-cover" /> )}
      ...
      {selectedCurrency && <CurrencySelector selectedCurrency={selectedCurrency} setSelectedCurrency={setSelectedCurrency} conversionRates={conversionRates} setConversionRates={setConversionRates} isAdmin={true} isEditing={isEditingCurrencies} setIsEditing={setIsEditingCurrencies} />}
        ...
        <div className="rounded-lg p-6 mb-6 bg-white w-full max-w-4xl mx-auto">
          ...
          {isEditingTabs ? (
            <div className="space-y-3 w-full">
              {editTabs.map((tab, index) => (
                <div key={index} className="flex items-center space-x-3">
                  <input type="text" className="input input-md w-full bg-gray-50 border" value={tab} onChange={(e) => { const newTabs = [...editTabs]; newTabs[index] = e.target.value; setEditTabs(newTabs); }}/>
            ...
            <div className="flex flex-wrap gap-3 justify-center ">
              {tabs.map((tab) => ( <Button key={tab} className={`btn-md px-5 font-semibold hover:bg-gray-300 shadow-equal ${ activeTab === tab ? "bg-gradient-to-r from-teal-400 via-cyan-500 to-blue-600 text-white" : "bg-white text-black no-border-tab "} capitalize rounded-lg`} onClick={() => setActiveTab(tab)}>{tab}</Button>))}
            </div>
          )}
          <div className="mt-5 relative">
            ...
            <svg className="absolute right-4 top-3.5 h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
          </div>
        </div>
      </AnimatedSection>
                ...
                <AnimatedSection>
                  <div className="group relative hover:scale-105 transition-all border p-3 rounded-lg max-w-xs mx-auto">
                    <div className="relative h-40 w-full mb-3">
                      {service.imageUrl && (service.imageUrl.endsWith(".mp4") || service.imageUrl.endsWith(".webm")) ? ( <video src={service.imageUrl} autoPlay loop muted playsInline className="object-contain h-full w-full" /> ) : ( <Image src={service.imageUrl || "/placeholder.png"} alt={service.title} width={200} height={160} className="object-contain h-full w-full" /> )}
                    </div>
                    <h2 className="font-bold text-xl mt-2 text-center">{service.title}</h2>
                    <p className="text-center text-sm text-gray-600 whitespace-pre-line">Price: {convertPrice(service.price || 0)} {selectedCurrency}{service.quantity && ` ${convertQuantityString(service.quantity)}`}</p>
                    <div className="absolute bottom-0 left-0 w-full p-3 bg-white opacity-0 group-hover:opacity-100 max-h-40 overflow-y-auto"><p className="text-xs sm:text-sm whitespace-pre-line">{convertDescriptionString(service.description, service.price)}</p></div>
                  </div>
      ...
      {isModalOpen && ( <dialog open className="modal modal-open z-50"><div className="modal-box max-w-3xl bg-white"><h3 className="font-bold text-lg">{currentService._id ? "Edit Service" : "Add New Service"}</h3><div className="py-4 space-y-4"><input type="text" className="input input-bordered w-full bg-white" name="title" value={currentService.title} onChange={handleInputChange} placeholder="Service Title" /><input type="number" className="input input-bordered w-full bg-white" name="price" value={currentService.price} onChange={handleInputChange} placeholder="Service Price" /><textarea className="textarea textarea-bordered h-24 bg-white w-full" name="quantity" value={currentService.quantity} onChange={handleInputChange} placeholder="Service Quantity" /><div><label className="block text-sm font-medium text-gray-700 mb-1">Service Media</label><div className="flex items-center gap-4"><input type="text" className="input input-bordered w-full bg-gray-100" name="imageUrl" value={currentService.imageUrl} onChange={handleInputChange} placeholder="Click 'Choose from Gallery' or paste URL" readOnly /><Button className="btn-secondary" onClick={() => setIsGalleryOpen(true)}>Choose from Gallery</Button></div></div>{previewUrl && ( <div className="mt-2 p-2 border rounded-md w-32"><p className="text-sm text-gray-500 mb-1">Preview:</p>{previewUrl.includes(".mp4") || previewUrl.includes(".webm") ? ( <video src={previewUrl} autoPlay loop muted playsInline className="max-h-40 object-contain mx-auto" /> ) : ( <img src={previewUrl} alt="Preview" className="max-h-40 object-contain mx-auto" /> )}</div> )}<textarea className="textarea textarea-bordered h-32 bg-white w-full" name="description" value={currentService.description} onChange={handleInputChange} placeholder="Service Description" /><select className="select select-bordered w-full bg-white" name="category" value={currentService.category} onChange={handleInputChange}>{tabs.map((tab) => (<option key={tab} value={tab}>{tab}</option>))}</select></div><div className="modal-action"><Button className="text-white bg-red-700 hover:bg-red-500" onClick={() => setIsModalOpen(false)}>Cancel</Button><Button className="btn-primary" onClick={handleSaveService}>Save</Button></div></div></dialog> )}
      {isGalleryOpen && ( <dialog open className="modal modal-open z-[51]"><div className="modal-box max-w-5xl bg-white"><h3 className="font-bold text-lg">Select Media from Gallery</h3><div className="py-4 grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-4 max-h-96 overflow-y-auto">{galleryItems.map((media) => ( <div key={media._id} className="group cursor-pointer" onClick={() => handleSelectFromGallery(media)}>{media.filetype === "video" ? ( <video src={media.url} muted className="w-full h-24 object-cover rounded-lg border-2 border-transparent group-hover:border-blue-500" /> ) : ( <img src={media.url} alt={media.filename} className="w-full h-24 object-cover rounded-lg border-2 border-transparent group-hover:border-blue-500" /> )}</div> ))}</div><div className="modal-action"><Button onClick={() => setIsGalleryOpen(false)}>Cancel</Button></div></div></dialog> )}
    </section>
  );
};
